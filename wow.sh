...
if [[ "$defaultVer" == *"$v3"* ]]; then
    python ./OwOParser.py $sourceCode && (python ./toJS.py $noWhite "$sourceCode.json") && rm "$sourceCode.json"
else
    pythonVer=$(which python3 2>/dev/null)
    v3="python3"
    if [[ "$pythonVer" == *"$v3"* ]]; then
        python3 ./OwOParser.py $sourceCode && (python3 ./toJS.py $noWhite "$sourceCode.json") && rm "$sourceCode.json"
    else
